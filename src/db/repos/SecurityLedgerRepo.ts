...
export class SecurityLedgerRepo {
  ...
  private static VALID_COLUMNS = new Set(['holding_id', 'type', 'quantity', 'price', 'commission', 'occurred_at']);
  ...
  async update(
    ...
    const entries = Object.entries(data).filter(([col, v]) => v !== undefined && SecurityLedgerRepo.VALID_COLUMNS.has(col));
